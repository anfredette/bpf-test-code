...
int tc_dispatcher(struct __sk_buff *skb)
{
	...
	if (num_progs_enabled < 1)
		goto out;
	ret = prog0(skb);
	bpf_trace_printk("fuc: %d, ret: %d, chain: 0x%08X", 50, 0, ret,
	...
	if (num_progs_enabled < 2)
		goto out;
	ret = prog1(skb);
	bpf_trace_printk("fuc: %d, ret: %d, chain: 0x%08X", 1, ret,
			 CONFIG.chain_call_actions[1]);
	...
	if (num_progs_enabled < 3)
		goto out;
	ret = prog2(skb);
	bpf_trace_printk("fuc: %d, ret: %d, chain: 0x%08X", 2, ret,
			 CONFIG.chain_call_actions[2]);
	...
	if (num_progs_enabled < 4)
		goto out;
	ret = prog3(skb);
	bpf_trace_printk("fuc: %d, ret: %d, chain: 0x%08X", 3, ret,
			 CONFIG.chain_call_actions[3]);
